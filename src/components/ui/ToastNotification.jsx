...
const TOAST_COLORS = {
  ...
  error: { bg: 'bg-red-500/10', border: 'border-red-500/20', text: 'text-red-400', icon: 'text-red-400' },
  info: { bg: 'bg-nexus-accent/10', border: 'border-nexus-accent/20', text: 'text-nexus-accent', icon: 'text-nexus-accent' },
};
...
function Toast({ toast, onDismiss }) {
  ...
  return (
    <div className={`flex items-start gap-2.5 px-3 py-2.5 rounded-md border transition-all duration-200 ${
      colors.bg} ${colors.border} ${exiting ? 'opacity-0 translate-x-4' : 'opacity-100 translate-x-0 animate-slide-in'
    }`}>
      <Icon size={14} className={`${colors.icon} mt-0.5 flex-shrink-0`} />
      <div className="flex-1 min-w-0">
        {toast.title && (
          <div className={`text-xs font-medium ${colors.text} mb-0.5`}>{toast.title}</div>
        )}
